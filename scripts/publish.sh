...
# Publish the Flexible Timers website to the live GitHub Pages repo.
#
# Source of truth : this repository (FlexibleTimers).
# Live deploy     : Samx2015.github.io/FlexibleTimers -> https://xintechllc.com/FlexibleTimers/
#
...
# path-scoped commit + push of the Pages repo, then verify the live URL.
#
# Usage:
#   scripts/publish.sh [--dry-run] [--no-verify]
#
# Environment overrides:
#   FLEXIBLETIMERS_PAGES_DIR   Live Pages folder
#                              (default: /Users/sam/GitHub/Samx2015.github.io/FlexibleTimers)
#   PUBLIC_BASE_URL            Base URL to verify
#                              (default: https://xintechllc.com/FlexibleTimers)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SOURCE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
DEST_DIR="${FLEXIBLETIMERS_PAGES_DIR:-/Users/sam/GitHub/Samx2015.github.io/FlexibleTimers}"
PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-https://xintechllc.com/FlexibleTimers}"
...
[[ -n "$SOURCE_DIR" && -n "$DEST_DIR" ]] || fail "empty source or destination path"
[[ "$SOURCE_DIR" != "/" && "$DEST_DIR" != "/" ]] || fail "refusing to sync filesystem root"
[[ -d "$SOURCE_DIR" ]] || fail "source missing: $SOURCE_DIR"
[[ -d "$DEST_DIR" ]] || fail "destination missing: $DEST_DIR (create the Pages folder first)"
[[ -f "$SOURCE_DIR/index.html" ]] || fail "source does not look like the website: $SOURCE_DIR"

IO_REPO="$(git -C "$DEST_DIR" rev-parse --show-toplevel 2>/dev/null)" \
  || fail "destination is not inside a git repo: $DEST_DIR"
...
log "Syncing Flexible Timers website -> $DEST_DIR"
...
if [[ -z "$(git -C "$IO_REPO" status --porcelain -- "$DEST_DIR")" ]]; then
  log "No website changes to publish."
else
  log "Committing and pushing Pages repo"
  git -C "$IO_REPO" add -- "$DEST_DIR"
  git -C "$IO_REPO" commit -q -m "Publish Flexible Timers website"
...
verify_url_contains "$PUBLIC_BASE_URL/" "Flexible Timers"
verify_url_contains "$PUBLIC_BASE_URL/support.html" "mailto:admin@xintechllc.com"
log "Flexible Timers website published: $PUBLIC_BASE_URL/"
